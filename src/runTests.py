...
for i in range(5):
    ...
    LSTM_dev_long, LSTM_test_long = LSTM.model.runLSTM(LSTM_train, LSTM_dev, LSTM_test, LSTM_token_vocab, EPOCH = 100, BATCH_SIZE = 32)
...
for i in range(5):
    ...
    BERT_dev_flat, BERT_test_flat = BERT.model.runBERT(BERT_train, BERT_dev, BERT_test, model_name, unique_tags, epochs = 10, BATCH_SIZE = 32, output = 0)
    BERT_dev_eval.evaluate(BERT_dev_flat, verbose = 0)
    BERT_test_eval.evaluate(BERT_test_flat, verbose = 0)
